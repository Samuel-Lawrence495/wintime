#' Fit a Markov model
#'
#' This function fits a Markov model to calculate the state probabilities for each arm. In the wintime package, the returned state
#' probability distributions are used in all non-pairwise methods. The Markov model is recommended for the Expected win time against
...
#' @param Time A `m x (n0 + n1)` matrix of event times. Rows should represent events and columns should represent participants. Event
...
#' a vector of unique control arm event times, a vector of unique treatment arm event times, the number of unique control
#' arm event times, the number of unique treatment arm event times, the control arm max follow time, the treatment arm
#' max follow time.
#' @examples
#' # -----------------------------
#' # Example inputs
