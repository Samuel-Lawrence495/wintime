...
#' @param Time A `m x n` matrix of event times, where `m` is the number of events in the hierarchy, and `n` is the total number of trial participants.
...
#' @param model Optional. String value. The type of model used to calculate state distributions. Default depends on `type`.
#' @param resample Optional. String value. The resampling method run after the observed data calculation. Default depends on `type`.
#' @param resample_num Optional. The number of desired resamples. Default is 0.
#' @param rmst_restriction Required only for `type` = 'rmt'. The RMT cutoff time.
...
#' the total losses on treatment (pairwise methods only), and potentially a warning message for combinations of `type` and `model`/`resample`
